...
1. Проверить изменения, `npm run check`, `npm test` и `npm run test:server`. Последняя команда проверяет серверную валидацию через SSH, без записи заявок и отправки сообщений.
2. Для первоначальной настройки или изменения доступов: `node scripts/deploy.mjs prepare`. Загружает закрытый обработчик, конфигурацию, проверяет PHP и запускает добавочные миграции. Несовместимые миграции требуют отдельного плана.
3. `node scripts/deploy.mjs publish`: сохраняет архивы публичных файлов и серверного PHP-кода, загружает dist/ и server/ в отдельные закрытые каталоги, проверяет PHP и публикует обе части. Существующий config.json сохраняется; серверный код не попадает в публичную папку. Устанавливает одно задание cron, сохраняя другие.
